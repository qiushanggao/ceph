    ...
    std::set<inodeno_t> *consumed_inos)
{
       ...
       lp != metablob.lump_order.end(); ++lp)
  {
    ...
    fdout(4) << "inspecting lump " << frag_oid.name << dendl;
    ...
    if (r == -ENOENT) {
      ...
      fdout(4) << "failed to read OMAP header from directory fragment "
      ...
      fdout(4) << "frag " << frag_oid.name << " fnode old v" <<
    ...
    if (write_fnode && !dry_run) {
      fdout(4) << "writing fnode to omap header" << dendl;
      bufferlist fnode_bl;
    ...
    for (list<ceph::shared_ptr<EMetaBlob::fullbit> >::const_iterator fbi = fb_list.begin(); fbi != fb_list.end(); ++fbi) {
      ...
      if (r != 0) {
        fderr << "unexpected error reading fragment object "
             << frag_oid.name << ": " << cpp_strerror(r) << dendl;
        return r;
      }
    
      fdout(4) << "inspecting fullbit " << frag_oid.name << "/" << fb.dn << dendl;
      bool write_dentry = false;
      if (vals.find(key) == vals.end()) {
        fdout(4) << "dentry did not already exist, will create" << dendl;
        write_dentry = true;
      } else {
        fdout(4) << "dentry " << key << " existed already" << dendl;
        assert(vals.size() == 1);
        fdout(4) << "dentry exists, checking versions..." << dendl;
        ...
        if (dentry_type == 'L') {
          ...
          fdout(4) << "skipping hardlink dentry in backing store" << dendl;
        } else if (dentry_type == 'I') {
          // Read out inode version to compare with backing store
          InodeStore inode;
          inode.decode_bare(q);
          fdout(4) << "decoded embedded inode version "
          ...
          fdout(4) << "corrupt dentry in backing store, overwriting from journal" << dendl;
          write_dentry = true;
        }
      }

      if (write_dentry && !dry_run) {
        fdout(4) << "writing dentry " << key << " into frag " << frag_oid.name << dendl;
        ...
        if (r != 0) {
          fdout(0) << "error writing dentry " << key
  ...
  for (list<ceph::shared_ptr<EMetaBlob::fullbit> >::const_iterator p = metablob.roots.begin(); p != metablob.roots.end(); ++p) {
    ...
    fdout(4) << "updating root 0x" << std::hex << ino << std::dec << dendl;

    object_t root_oid = InodeStore::get_object_name(ino, frag_t(), ".inode");
    fdout(4) << "object id " << root_oid.name << dendl;
    ...
    if (r == -ENOENT) {
      ...
      if (magic == CEPH_FS_ONDISK_MAGIC) {
        ...
        fdout(4) << "magic bad: '" << magic << "'" << dendl;
        write_root_ino = true;
      }
    } else {
      fderr << "error reading root inode object " << root_oid.name
            << ": " << cpp_strerror(r) << dendl;
      return r;
    }

    if (write_root_ino && !dry_run) {
      fdout(4) << "writing root ino " << root_oid.name
      ...
      if (r != 0) {
        fderr << "error writing inode object " << root_oid.name
              << ": " << cpp_strerror(r) << dendl;
        return r;
      }
    }
...
int JournalTool::consume_inos(const std::set<inodeno_t> &inos)
{
      ...
      rank_i != in_ranks.end(); ++rank_i)
  {
    ...
    if (read_r < 0) {
      ...
      fderr << "unable to read inotable '" << inotable_oid.name << "': "
        ...
        i != inos.end(); ++i)
    {
      ...
      if (projected_free.contains(ino)) {
        fdout(4) << "Used ino 0x" << std::hex << ino << std::dec
    ...
    if (inotable_modified) {
      inotable_ver += 1;
      fdout(4) << "writing modified inotable version " << inotable_ver << dendl;
      ...
      if (write_r != 0) {
        fderr << "error writing modified inotable " << inotable_oid.name
